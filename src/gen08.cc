...
void Gen::g_range(const Range &range) {
    ...
    case Range::kSimpleRange: {
        const auto &simpRange = range.simplerange();
        g_simple_expression(simpRange.left());
        out_ << " ";
        g_direction(simpRange.direction());
        out_ << " ";
        g_simple_expression(simpRange.right());
        break;
    }
...
void Gen::g_record_type_definition(const RecordTypeDefinition &recTypeDef) {
    if (recTypeDef.elems().empty()) {
        throw std::invalid_argument("ElementDeclaration is empty");
    }
    out_ << "record\n";
...
void Gen::g_constant_declaration(const ConstantDeclaration &constDecl) {
    ...
    if (constDecl.has_initexpr()) {
        out_ << " := ";
        g_expression(constDecl.initexpr());
    }
    out_ << ";\n";
}
...
void Gen::g_interface_constant_declaration(const InterfaceConstantDeclaration &constDecl) {
    ...
    out_ << " ";
    g_subtype_indication(constDecl.typeind());
    if (constDecl.has_initexpr()) {
        out_ << " := ";
        g_expression(constDecl.initexpr());
    }
...
void Gen::g_interface_signal_declaration(const InterfaceSignalDeclaration &sigDecl) {
    ...
    g_subtype_indication(sigDecl.typeind());
    if (sigDecl.isbus()) {
        out_ << " bus";
    }
    if (sigDecl.has_initexpr()) {
...
void Gen::g_interface_variable_declaration(const InterfaceVariableDeclaration &varDecl) {
    ...
    out_ << " ";
    g_subtype_indication(varDecl.typeind());
    if (varDecl.has_initexpr()) {
        out_ << " := ";
        g_expression(varDecl.initexpr());
    }
...
void Gen::g_interface_type_declaration(const InterfaceTypeDeclaration &typeDecl) {
    g_interface_incomplete_type_declaration(typeDecl.incompletetypedecl());
}

void Gen::g_interface_incomplete_type_declaration(const InterfaceIncompleteTypeDeclaration &typeDecl) {
    out_ << "type ";
...
void Gen::g_formal_designator(const FormalDesignator &formalDesignator) {
    ...
    case FormalDesignator::kIsGeneric:
...
void Gen::g_actual_designator(const ActualDesignator &actualDesignator) {
    ...
    case ActualDesignator::kExpr:
        g_expression(actualDesignator.expr());
        break;
...
void Gen::g_external_constant_name(const ExternalConstantName &extConstName) {
    ...
    out_ << " : ";
    g_subtype_indication(extConstName.typeind());
    out_ << " >>";
}
...
void Gen::g_external_signal_name(const ExternalSignalName &extSigName) {
    ...
    out_ << " : ";
    g_subtype_indication(extSigName.typeind());
    out_ << " >>";
}
...
void Gen::g_external_variable_name(const ExternalVariableName &extVarName) {
    ...
    out_ << " : ";
    g_subtype_indication(extVarName.typeind());
    out_ << " >>";
}
...
void Gen::g_factor(const Factor &factor) {
    switch (factor.unary_case()) {
    case Factor::kSimpUnary: {
        const auto &simpUnary = factor.simpunary();
        if (simpUnary.has_primaryexp()) {
            g_primary(simpUnary.primaryexp());
            out_ << " ** ";
        }
        break;
    }
    ...
    case Factor::kIsNot:
        out_ << "not ";
        break;
    case Factor::kLogicOp:
        g_logical_operator(factor.logicop());
        out_ << " ";
        break;
    case Factor::UNARY_NOT_SET:
        throw std::invalid_argument("Factor not set");
    }
    g_primary(factor.primary());
}
...
void Gen::g_miscellaneous_operator(const MiscellaneousOperator op) {
    switch (op) {
    case MiscellaneousOperator::EXP:
        out_ << "**";
        break;
    case MiscellaneousOperator::ABS:
        out_ << "abs";
        break;
    case MiscellaneousOperator::NOT:
        out_ << "not";
        break;
    default:
        throw std::invalid_argument("Unknown MiscellaneousOperator");
        break;
    }
}
...
void Gen::g_function_call(const FunctionCall &funcCall) {
    ...
    if (funcCall.has_actualparampart()) {
        out_ << " (";
        g_actual_parameter_part(funcCall.actualparampart());
        out_ << ")";
    }
}

void Gen::g_actual_parameter_part(const ActualParameterPart &actParamPart) {
    g_association_list(actParamPart.assoclist());
}
...
void Gen::g_conditional_signal_assignment(const ConditionalSignalAssignment &condSigAssign) {
    switch (condSigAssign.assignment_case()) {
    case ConditionalSignalAssignment::kCondWaveAssign:
        g_conditional_waveform_assignment(condSigAssign.condwaveassign());
        break;
    case ConditionalSignalAssignment::kCondForceAssign:
        g_conditional_force_assignment(condSigAssign.condforceassign());
        break;
    case ConditionalSignalAssignment::ASSIGNMENT_NOT_SET:
        throw std::invalid_argument("ConditionalSignalAssignment not set");
        break;
    }
}

void Gen::g_conditional_waveform_assignment(const ConditionalWaveformAssignment &condWaveAssign) {
    g_target(condWaveAssign.target());
    out_ << " <= ";
    if (condWaveAssign.has_delaymech()) {
        g_delay_mechanism(condWaveAssign.delaymech());
        out_ << " ";
    }
    g_conditional_waveforms(condWaveAssign.condwaveforms());
    out_ << ";\n";
}
...
void Gen::g_conditional_force_assignment(const ConditionalForceAssignment &condForceAssign) {
    g_target(condForceAssign.target());
    out_ << " <= force ";
    if (condForceAssign.has_mode()) {
        g_force_mode(condForceAssign.mode());
        out_ << " ";
    }
    g_conditional_expressions(condForceAssign.condexprs());
    out_ << ";\n";
}

void Gen::g_conditional_expressions(const ConditionalExpressions &condExprs) {
    g_expression(condExprs.expression());
    out_ << " when ";
    g_condition(condExprs.condition());
    for (const auto &item : condExprs.condexprs()) {
        out_ << " else ";
        g_expression(item.expression());
        out_ << " when ";
        g_condition(item.condition());
    }
    if (condExprs.has_elseexpr()) {
        out_ << " else ";
        g_expression(condExprs.elseexpr());
    }
}
...
void Gen::g_variable_assignment_statement(const VariableAssignmentStatement &varAssignStmt) {
    ...
    case VariableAssignmentStatement::kSimpleVarAssign:
        ...
        break;
    case VariableAssignmentStatement::kCondVarAssign:
        g_conditional_variable_assignment(varAssignStmt.condvarassign());
        break;
...
void Gen::g_simple_variable_assignment(const SimpleVariableAssignment &simpleVarAssign) {
    ...
    out_ << " := ";
    g_expression(simpleVarAssign.expr());
    out_ << ";\n";
}

void Gen::g_conditional_variable_assignment(const ConditionalVariableAssignment &condVarAssign) {
    g_target(condVarAssign.target());
    out_ << " := ";
    g_conditional_expressions(condVarAssign.condexprs());
    out_ << ";\n";
}
...
void Gen::g_procedure_call(const ProcedureCall &procCall) {
    ...
    if (procCall.has_actualparampart()) {
        out_ << " ";
        g_actual_parameter_part(procCall.actualparampart());
    }
}
...
void Gen::g_return_statement(const ReturnStatement &returnStmt) {
    if (returnStmt.has_label()) {
        g_label(returnStmt.label());
        out_ << ": ";
    }
    out_ << "return ";
    g_expression(returnStmt.expr());
    out_ << ";\n";
}
...
void Gen::g_bit_string_literal(const BitStringLiteral &bitStrLit) {
    if (bitStrLit.has_size()) {
        const auto &size = bitStrLit.size();
        if (size > 2048) {
            throw std::invalid_argument("BitStringLiteral with size > 2048");
        }
        out_ << size;
    }
